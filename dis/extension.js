...
function activate(context) {
    const cache = new cache_1.TranslationCache(context.globalState);
    // 설정 가져오기
    function getConfig() {
        const config = vscode.workspace.getConfiguration('prompt-translate-lens');
        return {
            targetLanguage: config.get('targetLanguage', 'ko'),
            googleApiKey: config.get('googleApiKey', '')
        };
    }
    // 캐시 클리어 커맨드
    context.subscriptions.push(vscode.commands.registerCommand('prompt-translate-lens.clearCache', async () => {
        await cache.clear();
